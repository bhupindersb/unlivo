...
export default function ConfirmPage() {
  ...
  useEffect(() => {
    ...
    if (!client) { setError("Authentication is temporarily unavailable. Please try again."); setStatus("error"); return; }
    let cancelled = false;
    ...
    const finish = async () => {
      try {
        ...
        const profilePayload: Record<string, any> = { id: user.id, email: user.email ?? null, updated_at: new Date().toISOString() };
        if (queryType === "signup" || (!queryType && !code)) {
          profilePayload.full_name = [metadata.first_name, metadata.last_name].filter(Boolean).join(" ").trim() || null;
          profilePayload.phone = typeof metadata.phone === "string" && metadata.phone.trim() ? metadata.phone.trim() : null;
        }
        ...
        if (!cancelled) { setConfirmedType(queryType ?? "signup"); setStatus("success"); }
      } catch (e: any) {
        if (cancelled) return;
        setError(e?.message || "This confirmation link is incomplete or has expired."); setStatus("error");
      }
    };
    finish();
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[#f7fafb] px-6 text-[#102638]"><div className="w-full max-w-md rounded-3xl border border-[#dfe9ed] bg-white p-8 text-center shadow-[0_18px_55px_rgba(16,38,56,0.08)]">
      ...
      <p className="mt-3 text-sm leading-6 text-[#687987]">{status === "error" ? error : status === "success" ? confirmedType === "invite" ? "Your UNLIVO staff invitation has been confirmed. You can now continue to your staff login." : "Your email address has been verified. For your security, please log in to continue." : "Please wait while we securely verify your email…"}</p>
