...
export default function PropertiesPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      let query = supabase
        .from("properties")
        .select("id,title,city,locality,price,rent_monthly,bedrooms,bathrooms,area_sqft,category,purpose")
        .eq("status","published")
        .eq("purpose",purpose)
      ...
      if (locationParam) {
        const location = locationParam.trim().replace(/,/g, " ");
        query = query.or(`city.ilike.%${location}%,locality.ilike.%${location}%`);
      }
      if (budgetParam) {
        const [min,max] = budgetParam.split("-").map(Number);
        if (purpose === "sale" && Number.isFinite(min)) query = query.gte("price",min);
        if (purpose === "sale" && Number.isFinite(max)) query = query.lte("price",max);
        if (purpose === "rent" && Number.isFinite(min)) query = query.gte("rent_monthly",min);
        if (purpose === "rent" && Number.isFinite(max)) query = query.lte("rent_monthly",max);
      }
