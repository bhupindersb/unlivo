    ...
    <form onSubmit={submit} className="rounded-2xl border border-white/70 bg-white p-4 shadow-soft sm:p-5">
      <div className="mb-4 flex flex-wrap items-center gap-x-8 gap-y-3 border-b border-[#edf1f3] pb-3 text-[13px] font-bold">
        {tabs.map(({ label, value, icon: Icon }) => (
          <button type="button" key={value} onClick={() => chooseTab(value)} className={`flex cursor-pointer items-center gap-2 border-b-2 pb-2 transition ${((value === "sale" || value === "rent") ? purpose === value : category === value) ? "border-[#102638] text-[#102638]" : "border-transparent text-[#5d6d7c]"}`}>
            <Icon size={20} />{label}
          </button>
        ))}
      </div>
      <div className="grid gap-3 md:grid-cols-[1.5fr_.62fr_.62fr_auto]">
        ...
        <label className="relative flex min-h-[48px] cursor-pointer items-center rounded-lg border border-[#dce5ea] bg-white px-4 text-[13px] text-[#193246]">
          <select value={category} onChange={(e) => setCategory(e.target.value)} className="w-full cursor-pointer appearance-none bg-transparent outline-none">
        ...
        <label className="relative flex min-h-[48px] cursor-pointer items-center rounded-lg border border-[#dce5ea] bg-white px-4 text-[13px] text-[#193246]">
          <select value={budget} onChange={(e) => setBudget(e.target.value)} className="w-full cursor-pointer appearance-none bg-transparent outline-none">
