    ...
    <header className={`${overlay ? "absolute left-0 right-0 top-0 z-30" : "border-b bg-white"}`}>
      <div className="container flex h-[78px] items-center justify-between gap-5">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          {nav.map((item) => <a key={item} href={item === "Sell" ? "/post-property" : "/properties"} className="cursor-pointer text-[13px] font-medium text-[#193246] transition hover:text-[#08aa91]">{item}</a>)}
        ...
        <div className="hidden items-center gap-3 md:flex">
          {user ? (
            <div className="relative">
              <button onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-haspopup="menu" className="inline-flex h-11 cursor-pointer items-center gap-2 rounded-full border border-[#d7e3e8] bg-white px-4 text-[13px] font-bold text-[#193246] transition hover:border-[#0bb89b] hover:shadow-sm">
                <UserCircle size={18} className="text-[#087f73]" />
                <span className="max-w-[145px] truncate">Welcome, {name}</span>
                <ChevronDown size={16} className={`transition-transform ${open ? "rotate-180" : ""}`} />
              </button>
              {open && (
                <div className="absolute right-0 top-[52px] z-50 w-60 overflow-hidden rounded-2xl border border-[#dfe9ed] bg-white p-2 shadow-[0_18px_50px_rgba(16,38,56,0.14)]" role="menu">
                  ...
                  <div className="py-1">
                    <a href="/my-properties" onClick={() => setOpen(false)} className="flex items-center rounded-xl px-3 py-2.5 text-sm font-semibold text-[#193246] hover:bg-[#f3f8f9]">My Properties</a>
                    <a href="/profile" onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-xl px-3 py-2.5 text-sm font-semibold text-[#193246] hover:bg-[#f3f8f9]"><UserCircle size={16} /> Profile</a>
                    {(role === "admin" || role === "reviewer") && <a href={staffHref} onClick={() => setOpen(false)} className="flex items-center rounded-xl px-3 py-2.5 text-sm font-semibold text-[#193246] hover:bg-[#f3f8f9]">{staffLabel}</a>}
                    <a href="/post-property" onClick={() => setOpen(false)} className="mt-1 flex items-center justify-center rounded-xl bg-[#071d2d] px-3 py-2.5 text-sm font-bold text-white hover:bg-[#102f44]">Post Property</a>
                  </div>
            ...
            <><a href="/login" className="cursor-pointer px-3 py-2 text-[13px] font-semibold text-[#193246]">Login</a><a href="/signup" className="cursor-pointer rounded-full bg-[#071d2d] px-6 py-3 text-[13px] font-bold text-white shadow-lg hover:bg-[#102f44]">Sign Up</a></>
          )}
        </div>
      ...
      {mobileOpen && (
        <div className="border-t bg-white px-5 py-5 shadow-lg md:hidden">
          <div className="container grid gap-3">
            {nav.map((item) => <a key={item} href={item === "Sell" ? "/post-property" : "/properties"} className="rounded-lg px-3 py-3 text-sm font-semibold">{item}</a>)}
